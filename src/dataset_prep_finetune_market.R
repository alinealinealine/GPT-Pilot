...
AIMM_text$project_description <- gsub(pattern = "\032;|\032",replacement = "",x = AIMM_text$project_description)

AIMM_text$prompt <- paste0(str_squish(AIMM_text$project_description),"\n\n###\n\n")
AIMM_text$prompt <- paste0(str_squish(AIMM_text$project_description)," The Project is in IFC sector ",str_squish(AIMM_text$sector)," and in ",str_squish(AIMM_text$country_description),".","\n\n###\n\n")

AIMM_text$completion <- paste0(" ",str_squish(AIMM_text$text.dev_impct_m),"\n[END]")
...
jsonlite::stream_out(x = temp[,c("prompt","completion")],file("./output/ALL-BP-MAR-0001.json"))
